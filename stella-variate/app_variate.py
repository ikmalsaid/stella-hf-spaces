...
with ui.Blocks(css=css, title=title(feature), theme=theme, analytics_enabled=False) as stella:
    ...
    with ui.Group():
        input_variate = ui.Image(type='pil', show_label=False, height="40vh", sources=['upload'])
        output_variate = ui.Gallery(show_label=False, object_fit="contain", height="50vh", show_share_button=False)
        prompt_variate = ui.Textbox(label=sprompt1, placeholder=spholder)
        with ui.Row():
